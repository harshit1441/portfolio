...
const Title = styled.h2`
  font-size: 3.5rem; // or even 3rem for more impact
  color: white;
  margin-bottom: 20px;
`;
...
const skillsData = [
  ...
  { name: "Firebase", component: FirebaseIcon },
...
const Skills = () => {
  return (
    <Container>
      ...
      <Description>
        I specialize in frontend and backend development, using modern tools and frameworks to build sleek, scalable web applications.
      </Description>
      <Grid>
        {skillsData.map((skill, index) => (
          <SkillCard key={index} title={skill.name}>
            {skill.icon ? (
              <i className={`bx ${skill.icon}`}></i>
            ) : skill.component ? (
              <skill.component style={{ width: 48, height: 48 }} />
            ) : null}
          </SkillCard>
        ))}
      </Grid>
